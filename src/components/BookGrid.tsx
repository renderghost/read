...
const BookGrid: React.FC<BookGridProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    if (bookSlug) {
      const foundBook = books.find(
        (book) => book.title.toLowerCase().replace(/\s+/g, '-') === bookSlug
      );
  ...
  return (
    <>
      <div className={`bones-book-grid flex-grow ${className}`}>
        <div
          className="
            grid
            w-full
...
export const BookGridSkeleton: React.FC<{ className?: string }> = ({ className = "" }) => (
  <div className={`bones-book-grid-skeleton flex-grow ${className}`}>
    <div className="grid w-full gap-2 md:gap-4 grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 max-w-7xl mx-auto">
      {Array(21).fill(0).map((_, index) => (
        <div 
          key={index}
          className="bones-book-card-skeleton relative aspect-[33/50] w-full overflow-hidden rounded-md bg-bones-gainsboro dark:bg-bones-slategray animate-pulse"
        ></div>
      ))}
    </div>
  </div>
